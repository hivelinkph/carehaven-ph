...
export default function AdminEditFacilityPage({ params }: { params: Promise<{ id: string }> }) {
    const { id } = use(params);
    ...
    return (
        <div className="min-h-screen bg-[#faf9f5] pt-24 pb-12">
            <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                <Link
                    href="/dashboard"
                    className="inline-flex items-center gap-2 text-sm text-[#b0aea5] hover:text-[#d97757] mb-6 transition-colors"
                ...
                {loading ? (
                    ...
                    <div className="glass-card overflow-hidden">
                        <div className="p-6 md:p-8 bg-[#d97757]/5 border-b border-[#e8e6dc]/50">
                            <h1 className="text-2xl font-bold text-[#2D3748]" style={{ fontFamily: "var(--font-heading)" }}>
                                Admin Edit: {facility.name}
                            </h1>
                            <p className="text-[#b0aea5] mt-1" style={{ fontFamily: "var(--font-ui)" }}>
                                You are modifying this facility as an administrator.
                            </p>
                        </div>
                        <div className="p-6 md:p-8">
                            <FacilityForm mode="edit" facility={facility} isAdmin={true} />
                        </div>
                    </div>
                )}
            </div>
